...
Rails.logger.debug 'Creating cities...'
# Florida cities
...
florida.cities.create!(name: 'Jacksonville', latitude: 30.3322, longitude: -81.6557)
florida.cities.create!(name: 'Tallahassee', latitude: 30.4383, longitude: -84.2807)
...
texas.cities.create!(name: 'San Antonio', latitude: 29.4241, longitude: -98.4936)
texas.cities.create!(name: 'Fort Worth', latitude: 32.7555, longitude: -97.3308)
...
california.cities.create!(name: 'Sacramento', latitude: 38.5816, longitude: -121.4944)
california.cities.create!(name: 'San Jose', latitude: 37.3382, longitude: -121.8863)

Rails.logger.debug 'Creating service categories...'
...
cities.each_with_index do |city, idx| # rubocop:disable Metrics/BlockLength
  company = Company.create!(
    ...
    street_address: "#{rand(100..9999)} #{%w[Main Oak Pine Maple Cedar].sample} St",
    ...
    service_level: %w[Basic Standard Premium Elite].sample,
    description: "Professional sewer and drain services with over #{rand(5..25)} years of experience. Licensed, insured, and available 24/7 for emergency services.", # rubocop:disable Metrics/LineLength
...
Rails.logger.debug 'Creating gallery images...'
Rails.logger.debug 'Note: Gallery images created without files attached. Use Active Storage to attach actual images.'
companies.each do |company|
  ...
  rand(3..7).times do |i|
    ...
    begin
      ...
      Rails.logger.debug { "  Skipping image download: #{e.message}" }
    end
  end
end

Rails.logger.debug 'Creating certifications...'
cert_names = [
...
companies.each do |company|
  rand(1..3).times do
    company.certifications.create!(
      ...
      certificate_number: "CERT-#{rand(100_000..999_999)}",
...
Rails.logger.debug { "Countries: #{Country.count}" }
Rails.logger.debug { "States: #{State.count}" }
Rails.logger.debug { "Cities: #{City.count}" }
Rails.logger.debug { "Service Categories: #{ServiceCategory.count}" }
Rails.logger.debug { "Companies: #{Company.count}" }
